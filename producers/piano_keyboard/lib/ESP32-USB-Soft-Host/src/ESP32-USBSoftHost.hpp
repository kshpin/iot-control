...
void (*printDataCB)(uint8_t usbNum, uint8_t byte_depth, uint8_t *data,
...
static void Default_USB_DetectCB(uint8_t usbNum, void *dev) {
    ...
    // if( device->idProduct == mySupportedIdProduct && device->idVendor ==
    // mySupportedVendor ) {
    //   myListenUSBPort = usbNum;
    // }
}

static void Default_USB_DataCB(uint8_t usbNum, uint8_t byte_depth,
...
class USB_SOFT_HOST {
   public:
    bool init(usb_pins_config_t pconf,
              ondetectcb_t detectCB = Default_USB_DetectCB,
              printcb_t onDataCB = Default_USB_DataCB,
              ontick_t onTickCB = nullptr);
    ...
    // use those to avoid the pesky "Guru Meditation Error: Core 1 panic'ed
    // (Cache disabled but cached memory region accessed)" error may happen when
    // using SPIFFS, SD or other IRAM driven libraries
    void TimerPause();
    void TimerResume();
...
bool USB_SOFT_HOST::init(usb_pins_config_t pconf, ondetectcb_t onDetectCB,
                         printcb_t onDataCB, ontick_t onTickCB) {
    ...
    if (_init(pconf)) {
        xTaskCreatePinnedToCore(USB_SOFT_HOST::TimerTask,
                                "USB Soft Host Timer Task", 8192, NULL,
                                priority, NULL, core);
        // log_w("USB Soft Host Group timer task is now running on core #%d with
        // priority %d", core, priority);
        return true;
    }
    return false;
}
...
bool USB_SOFT_HOST::_init(usb_pins_config_t pconf) {
    ...
    config.auto_reload =
        (timer_autoreload_t)1;  // fix for ¬invalid conversion from 'int' to
                                // 'timer_autoreload_t'¬ thanks rudi ;-)
    ...
    initStates((gpio_num_t)pconf.dp0, (gpio_num_t)pconf.dm0,
    ...
    timer_set_alarm_value(TIMER_GROUP_0, TIMER_0,
                          (double)TIMER_INTERVAL0_SEC * TIMER_SCALE);
...
void USB_SOFT_HOST::setPrintCb(printcb_t onDataCB) { set_print_cb(onDataCB); }
...
void USB_SOFT_HOST::onUSBMessageDecode(uint8_t src, uint8_t len,
                                       uint8_t *data) {
